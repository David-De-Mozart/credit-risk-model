...
class RFMTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if 'TransactionStartTime' not in X.columns:
            raise ValueError("DataFrame is missing 'TransactionStartTime' column")
        ...
        frequency = X.groupby('CustomerId').size().reset_index(name='Frequency')
        
        # Only consider positive amounts for monetary value
        monetary = X[X['Amount'] > 0].groupby('CustomerId')['Amount'].sum().reset_index(name='Monetary')
        
        # Merge features
        rfm = recency.merge(frequency, on='CustomerId').merge(monetary, on='CustomerId', how='left')
...
class FeatureAggregator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        agg_df.columns = [col.replace('get_mode', 'mode') for col in agg_df.columns]
